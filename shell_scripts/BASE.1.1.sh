...
alias phyutility="java -jar ~/bin/phyutility.jar"
...
BASE has been tested on CentOS 7.0 and relies on the following softwares:
...
Each software has or (A) to be in the PATH (B) be installed with conda. 
...
Phyutility has no conda-installation, so the java executable available at 

	https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/phyutility/phyutility.jar 

shoud be placed in  ~/bin/. The correct installation and versions of the requirements can be checked using the --requirements option.
...
do
cp RAxML_result.$f".tre" RAxML_result.$f".$lab.tre"

while read line ; do 

number_of_species=$(echo $line | awk -F " "  '{$NF=""; print $0}'| wc -w );
species=$(echo $line | awk -F " "  '{$NF=""; print $0}' | tr -d " ")
...
then for q in $(cat $j"missing_otus.lst"); do if [[ $(echo $line | grep "$q") ]]; then let missing=missing+1; fi; done 

fi

number_of_parentheses=$((number_of_species -missing -1));

if [[ $number_of_parentheses == 0 ]] && [[ $number_of_species -eq 1 ]]; 

then sed -i "s/$species/$species$tag/" RAxML_result.$f".$lab.tre";

continue

elif [[ $number_of_parentheses == 0 ]] && [[ $number_of_species -gt 1 ]];

  then echo -e "  just one OTU of the group $tag is present in gene $j \n" >> ../warnings_summary.txt; 

  continue 

  elif [[ $number_of_parentheses -lt 0 ]];

then echo -e "  no OTU of the group $tag is present in gene $j \n" >> ../warnings_summary.txt;

continue

fi

#  echo $j $rep $line $number_of_species $missing $number_of_parentheses
#  tag=$(echo $line | awk -F " " '{print $NF}');
#  echo $number_of_species $number_of_parentheses $tag

for i in $(seq 1 $number_of_species);

do export a=$(echo $line | awk -F " "  '{$NF=""; print $0}' | awk -v var="$i" '{ print $var }')

echo "-e \"$a\" " | tr -d '\n' >> grep_argument.tmp

done;

start_point=$(sed 's/,/,\'$'\n/g' RAxML_result.$f".$lab.tre" | sed 's/)/)\'$'\n/g' | sed 's/(/(\'$'\n/g' | eval grep $(cat grep_argument.tmp) | head -1 | awk -F ":" '{print $1}');

ciao=$(printf %.0s'[^)]*)' $(seq 1 $number_of_parentheses));

sed -i 's/'$start_point''$ciao'/&'$tag'/' RAxML_result.$f".$lab.tre"
...
bad_tag=$(echo $tree_end | tr -d "0.:,;")

good_tree_end=:0.0\;

if [[ $tree_end != $good_tree_end ]];

then echo -e "  impossible to use tag $bad_tag in gene $j because of topology \n" >> ../warnings_summary.txt;

sed -i "s/$bad_tag//" RAxML_result.$f".$lab.tre";
...
if [[ -f failed_clusters.txt ]]; then if [[ $(wc -l failed_clusters.txt | awk '{print $1}') == $tot_genes ]]; then echo -e "\n all clusters failed, check the warning_summary.txt file in the .tmp.full.out folder. \n" ; exit; fi; fi
...
printf 'likelihood.summary$p.value <- round(adj.p.value,digits=4) \n' >> LRT.R
...
if [[ -a warnings_summary.txt ]]; then echo -e "  the analysis has produced some warning: you will find the information relative to each failure in the file warning_summary.txt"; fi

cd ../$output_folder

######################################################################################## builds files for branches when there are no missing data

if  [ "$missing_data" != 1 ] && [ -z "$labels" ]; then
