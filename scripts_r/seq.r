...
aln <- readSet(file="substitute_this")
...
sink("result.tmp")
cat(c(BAT.result$W.to.S, BAT.result$S.to.W, BRO.result$W.to.S, BRO.result$S.to.W, BGM.result$W.to.S, BGM.result$S.to.W, bgc.BAT, bgc.BRO, bgc.BGM, enc.BAT, enc.BRO, enc.BGM, scuo.BGM, scuo.BRO, scuo.BGM))
cat("\n")
sink()
